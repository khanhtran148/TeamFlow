...
public sealed class DeleteProjectTests
{
    ...
    public async Task Handle_InsufficientPermission_ReturnsForbidden()
    {
        ...
        var result = await CreateHandler().Handle(new DeleteProjectCommand(Guid.NewGuid()), CancellationToken.None);
    ...
    public async Task Handle_PermissionCheckedWithProjectEditPermission()
    {
        ...
        await CreateHandler().Handle(new DeleteProjectCommand(project.Id), CancellationToken.None);
